    ...
    <div class="products-grid">
        ...
        <a href="{{ route('buyer.view_product', $art->id) }}">
            <div class="product-card">
                ...
                <!-- // Ensure the path matches the format stored in the database -->
                    @php
                       
                        $imagePath = str_starts_with($art->image, 'arts/') ? 'storage/' . $art->image : 'storage/arts_images/' . $art->image;
                    @endphp

                    @if($art->image && file_exists(storage_path('app/public/' . $imagePath)))
                        <img src="{{ asset($imagePath) }}" alt="{{ $art->title }}">
                    @else
